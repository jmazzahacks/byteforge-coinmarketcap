...
class Market(object):
	...
	def quotes_historical_v2(self, ticker: str, timestamp_start: int, timestamp_end: int, interval: str = '24h') -> List[TokenState]:
		return _quotes_historical_v2(self, ticker, timestamp_start, timestamp_end, interval)
	...
	def listings_latest(self, sort_by: SortOption = SortOption.MARKET_CAP, sort_dir: SortDir = SortDir.DESC, start: int = 1, limit: int = 100, convert: str = None, aux_fields: AuxFields = None, filters: FilterOptions = None) -> List[TokenState]:
		...
		response = self._request('v1/cryptocurrency/listings/latest', params=params)
