...
def test_dex_listings_info_multiple_ids(market_instance):
    ...
    # Uniswap v4 and v3 (Ethereum), verified via /v1/exchange/map.
    dex_list = market_instance.dex_listings_info(ids=[11955, 1348])

    assert isinstance(dex_list, list)
    assert {dex.id for dex in dex_list} == {11955, 1348}
    assert len(dex_list) == 2
